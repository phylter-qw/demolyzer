                ...
                if (match.PlayerCount > 1 && teamOnly == false)
                {
                    ...
                    for (int indexA = 0; indexA < teamPlayerCount; ++indexA)
                    {
                        ...
                        for (int indexB = 0; indexB < teamPlayerCount; ++indexB)
                        {
                            ...
                            float teamAELO = participantTeamA[indexA].ELO;
                            float teamBELO = participantTeamB[indexB].ELO;

                            float expectedAvsB = GetExpectedScore(teamAELO - teamBELO);
                            float expectedBvsA = GetExpectedScore(teamBELO - teamAELO);
                            teamAExpectedScores[indexA] += expectedAvsB;
                            teamBExpectedScores[indexB] += expectedBvsA;

                            //Debug.WriteLine(String.Format("{0}[{1:F0}] vs {2}[{3:F0}] = {4:F3}", playerA, teamAELO, playerB, teamBELO, expectedAvsB));
                        }
                    }
                    ...
                    //for (int index = 0; index < teamPlayerCount; ++index)
                    //{
                    //    Debug.WriteLine(String.Format("TeamA[{0}]={1:F2}", index, teamAExpectedScores[index]));
                    //}
                    //for (int index = 0; index < teamPlayerCount; ++index)
                    //{
                    //    Debug.WriteLine(String.Format("TeamB[{0}]={1:F2}", index, teamBExpectedScores[index]));
                    //}

                    //float teamAExpectedScore = 0f;
                    //float teamBExpectedScore = 0f;
                    //for (int index = 0; index < teamPlayerCount; ++index)
                    //{
                    //    teamAExpectedScore += teamAExpectedScores[index];
                    //    teamBExpectedScore += teamBExpectedScores[index];
                    //}
                    //teamAExpectedScore /= teamPlayerCount;
                    //teamBExpectedScore /= teamPlayerCount;
                    ...
                    //for (int index = 0; index < teamPlayerCount; ++index)
                    //{
                    //    Debug.WriteLine(String.Format("TeamAAdjustments[{0}]={1:F2}", index, teamATeammateFactors[index]));
                    //}
                    //for (int index = 0; index < teamPlayerCount; ++index)
                    //{
                    //    Debug.WriteLine(String.Format("TeamBAdjustments[{0}]={1:F2}", index, teamBTeammateFactors[index]));
                    //}

                    ////perform expected score adjustments
                    //for (int index = 0; index < teamPlayerCount; ++index)
                    //{
                    //    teamAExpectedScores[index] *= teamATeammateFactors[index];
                    //    teamBExpectedScores[index] *= teamBTeammateFactors[index];
                    //}
                    //asdf = 0;

                    //for (int index = 0; index < teamPlayerCount; ++index)
                    //{
                    //    Debug.WriteLine(String.Format("TeamAAdjusted[{0}]={1:F2}", index, teamAExpectedScores[index]));
                    //}
                    //for (int index = 0; index < teamPlayerCount; ++index)
                    //{
                    //    Debug.WriteLine(String.Format("TeamBAdjusted[{0}]={1:F2}", index, teamBExpectedScores[index]));
                    //}
                    ...
                    //float eloPointsTeamA = ELOCoefficient * (match.TeamAPercent - expectedTeamA);
                    //float eloPointsTeamB = ELOCoefficient * (match.TeamBPercent - expectedTeamB);
                    ...
                    //for (int index = 0; index < teamPlayerCount; ++index)
                    //{
                    //    float eloPoints = ELOCoefficient * (match.TeamAPercents[index] - teamATeammateFactors[index]);
                    //    Debug.WriteLine(String.Format("PlayerA[{0}] Expected={1:F2} Actual={2:F2} Points={3:F2} TotalPoints={4:F2}", index, teamATeammateFactors[index], match.TeamAPercents[index], eloPoints, eloPoints + eloPointsTeamA));
                    //}
                    //for (int index = 0; index < teamPlayerCount; ++index)
                    //{
                    //    float eloPoints = ELOCoefficient * (match.TeamBPercents[index] - teamBTeammateFactors[index]);
                    //    Debug.WriteLine(String.Format("PlayerB[{0}] Expected={1:F2} Actual={2:F2} Points={3:F2} TotalPoints={4:F2}", index, teamBTeammateFactors[index], match.TeamBPercents[index], eloPoints, eloPoints + eloPointsTeamB));
                    //}
                    ...
                    float[] teamAQ = new float[teamPlayerCount];
                    float[] teamBQ = new float[teamPlayerCount];

                    float teamAAvgElo = 0;
                    float teamBAvgElo = 0;
                    for (int index = 0; index < teamPlayerCount; ++index)
                    {
                        teamAQ[index] = (float)Math.Pow(10, participantTeamA[index].ELO / 400f);
                        teamBQ[index] = (float)Math.Pow(10, participantTeamB[index].ELO / 400f);
                        teamAAvgElo += participantTeamA[index].ELO;
                        teamBAvgElo += participantTeamB[index].ELO;
                    }
                    teamAAvgElo /= teamPlayerCount;
                    teamBAvgElo /= teamPlayerCount;

                    float teamATotalQ = (float)Math.Pow(10, teamAAvgElo / 400f);
                    float teamBTotalQ = (float)Math.Pow(10, teamBAvgElo / 400f);

                    float eloAverageTeamA = 0;
                    float eloAverageTeamB = 0;

                    float[] teamAExpectedScores2 = new float[teamPlayerCount];
                    float[] teamBExpectedScores2 = new float[teamPlayerCount];

                    for (int index = 0; index < teamPlayerCount; ++index)
                    {
                        teamAExpectedScores2[index] = teamAQ[index] / (teamAQ[index] + teamBTotalQ);
                        teamBExpectedScores2[index] = teamBQ[index] / (teamBQ[index] + teamATotalQ);
                    }
                    ...
                    for (int index = 0; index < teamPlayerCount; ++index)
                    {
                        ...
                        //float eloPoints = eloPointsTeamA + ELOCoefficient * (match.TeamAPercents[index] - teamATeammateFactors[index]);
                        float eloPoints = ELOCoefficient * (match.TeamAPercents[index] - teamAExpectedScores[index]);
                        ...
                        matchA.ExpectedScore = teamAExpectedScores[index] * totalFrags / (float)teamPlayerCount; //example: 0.5 * 100 / 4 = 12.5 frags
                        ...
                        matchA.ExpectedScoreA = (short)(teamAExpectedScores[index] * totalFrags / (float)teamPlayerCount); //example: 0.5 * 100 / 4 = 12.5 frags
                    ...
                    for (int index = 0; index < teamPlayerCount; ++index)
                    {
                        ...
                        //float eloPoints = eloPointsTeamB + ELOCoefficient * (match.TeamBPercents[index] - teamBTeammateFactors[index]);
                        float eloPoints = ELOCoefficient * (match.TeamBPercents[index] - teamBExpectedScores[index]);
                        ...
                        matchB.ExpectedScore = teamBExpectedScores[index] * totalFrags / (float)teamPlayerCount; //example: 0.5 * 100 / 4 = 12.5 frags
                        ...
                        matchB.ExpectedScoreA = (short)(teamBExpectedScores[index] * totalFrags / (float)teamPlayerCount); //example: 0.5 * 100 / 4 = 12.5 frags
